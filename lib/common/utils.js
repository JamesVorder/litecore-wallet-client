...
var crypto = Litecore.crypto;
var encoding = Litecore.encoding;
...
Utils.hashMessage = function(text) {
  ...
  ret = new Litecore.encoding.BufferReader(ret).readReverse();
  return ret;
};
...
Utils.privateKeyToAESKey = function(privKey) {
  ...
  $.checkArgument(Litecore.PrivateKey.isValid(privKey), 'The private key received is invalid');
  var pk = Litecore.PrivateKey.fromString(privKey);
  return Litecore.crypto.Hash.sha256(pk.toBuffer()).slice(0, 16).toString('base64');
};
...
Utils.deriveAddress = function(scriptType, publicKeyRing, path, m, network) {
  ...
  var publicKeys = _.map(publicKeyRing, function(item) {
    var xpub = new Litecore.HDPublicKey(item.xPubKey);
...
Utils.signRequestPubKey = function(requestPubKey, xPrivKey) {
  var priv = new Litecore.HDPrivateKey(xPrivKey).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).privateKey;
  return Utils.signMessage(requestPubKey, priv);
};

Utils.verifyRequestPubKey = function(requestPubKey, signature, xPubKey) {
  var pub = (new Litecore.HDPublicKey(xPubKey)).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).publicKey;
...
Utils.buildTx = function(txp) {
  var t = new Litecore.Transaction();
  ...
  if (txp.toAddress && txp.amount && !txp.outputs) {
    ...
    _.each(txp.outputs, function(o) {
      ...
      if (o.script) {
        t.addOutput(new Litecore.Transaction.Output({
          script: o.script,
          satoshis: o.amount
        }));
      } else {
  ...
  // Validate inputs vs outputs independently of Litecore
