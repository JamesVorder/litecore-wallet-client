...
Credentials.create = function(network) {
  ...
  x.network = network;
  x.xPrivKey = (new Litecore.HDPrivateKey(network)).toString();
...
Credentials.fromExtendedPublicKey = function(xPubKey, source, entropySourceHex, account, derivationStrategy, opts) {
  ...
  var x = new Credentials();
  x.xPubKey = xPubKey;
  x.entropySource = Litecore.crypto.Hash.sha256sha256(entropyBuffer).toString('hex');
  x.account = account;
  x.derivationStrategy = derivationStrategy;
...
Credentials.prototype._hashFromEntropy = function(prefix, length) {
  ...
  var b2 = Litecore.crypto.Hash.sha256hmac(b, new Buffer(prefix));
...
Credentials.prototype._expand = function() {
  ...
  if (this.xPrivKey) {
    var xPrivKey = new Litecore.HDPrivateKey.fromString(this.xPrivKey);
    ...
    this.entropySource = Litecore.crypto.Hash.sha256(requestDerivation.privateKey.toBuffer()).toString('hex');
  } else {
    var seed = this._hashFromEntropy('reqPrivKey', 32);
    var privKey = new Litecore.PrivateKey(seed.toString('hex'), network);
...
Credentials.prototype.getDerivedXPrivKey = function(password) {
  ...
  var xPrivKey = new Litecore.HDPrivateKey(this.getKeys(password).xPrivKey, this.network);
  var deriveFn = !!this.compliantDerivation ? _.bind(xPrivKey.deriveChild, xPrivKey) : _.bind(xPrivKey.deriveNonCompliantChild, xPrivKey);
...
Credentials.fromOldCopayWallet = function(w) {
  function walletPrivKeyFromOldCopayWallet(w) {
    // IN LWS, the master Pub Keys are not sent to the server, 
    ...
    var privKey = new Litecore.PrivateKey.fromBuffer(Litecore.crypto.Hash.sha256(seedBuf));
  ...
  var pkr = _.map(w.publicKeyRing.copayersExtPubKeys, function(xPubStr) {
    ...
    if (isMe) {
      ...
      requestDerivation = (new Litecore.HDPrivateKey(credentials.xPrivKey))
        .deriveChild(path).hdPublicKey;
    } else {
      // this 
      var path = Constants.PATHS.REQUEST_KEY_AUTH;
      requestDerivation = (new Litecore.HDPublicKey(xPubStr)).deriveChild(path);
    }

    // Grab Copayer Name
    var hd = new Litecore.HDPublicKey(xPubStr).deriveChild('m/2147483646/0/0');
