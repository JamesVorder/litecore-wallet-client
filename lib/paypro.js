...
PayPro.get = function(opts, cb) {
  ...
  http(opts, function(err, dataBuffer) {
    ...
    var addr = new Litecore.Address.fromScript(new Litecore.Script(scriptBuf), network);
...
PayPro._getPayProRefundOutputs = function(addrStr, amount) {
  ...
  var addr = new Litecore.Address(addrStr);

  var s;
  if (addr.isPayToPublicKeyHash()) {
    s = Litecore.Script.buildPublicKeyHashOut(addr);
  } else if (addr.isPayToScriptHash()) {
    s = Litecore.Script.buildScriptHashOut(addr);
  } else {
